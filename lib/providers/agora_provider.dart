...
class CallProvider extends ChangeNotifier {
  ...
  Stream<DocumentSnapshot> get userCall =>
      fireStore.collection('call').doc(auth.currentUser?.uid).snapshots();

  void makeCall(BuildContext context, UserModel receiverData,
      UserModel userData, bool isGroupChat) async {
    try {
      String callId = const Uuid().v1();
      CallModel senderCallData = CallModel(
        callerId: userData.uid,
        callerName: userData.fullName,
        callerPic: sharedPref.profilePicture,
